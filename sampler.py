...
class BaseSampler:
    ...
    def setup_dist(self, gpu_id=None):
        ...
        # 删除与分布式训练相关的部分
        self.num_gpus = num_gpus
        self.rank = 0  # 设置 rank 为 0，表示不使用分布式训练
...
class DifFaceSampler(BaseSampler):
    ...
    def sample_func_ir_aligned(
        ...
        def model_fn(x, t, **model_kwargs):
            out = self.model(x, t, **model_kwargs)
            ...
            # if "out_channels" in self.configs.model.params.__dict__.keys():
            #     if self.configs.model.params.out_channels == 6:
        ...
        noise_schedule = NoiseScheduleVP('discrete', betas=betas)
